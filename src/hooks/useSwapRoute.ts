...
export function useSwapRoute(
  ...
  useEffect(() => {
    const timer = setTimeout(() => setDebouncedAmount(amountIn), 300);
  ...
  const { data: route, isLoading, error, isFetching } = useQuery<SwapRoute>({
  ...
  return { route, isLoading: isLoading && enabled, isFetching, error };
}
